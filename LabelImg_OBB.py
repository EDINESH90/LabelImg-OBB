...
# # #######------------------------------- DINU EDIT --------------------------------------------
        ...
        self.saveFormatCombo = QComboBox()
        self.saveFormatCombo.addItems(["PASCALVOC", "YOLO", "YOLO-Corner"])
        saveFormatLayout = QHBoxLayout()
        saveFormatLayout.addWidget(QLabel("Save format:"))
        saveFormatLayout.addWidget(self.saveFormatCombo)
        saveFormatContainer = QWidget()
        saveFormatContainer.setLayout(saveFormatLayout)
        listLayout.addWidget(saveFormatContainer)
...
# # #######--------------------------------------------------------------------------------------
    ...
    def saveLabels(self, annotationFilePath):
        ...
        try:
            ...
            # return True
# # #######-------------------------------- DINU EDIT ------------------------------------------------
            selected_format = self.saveFormatCombo.currentText()
            if selected_format == "PASCALVOC":
                print('Saving as Pascal VOC XML')
                self.labelFile.savePascalVocFormat(
                    annotationFilePath, shapes, self.filePath, self.imageData,
                    self.lineColor.getRgb(), self.fillColor.getRgb()
                )
            elif selected_format == "YOLO":
                # print('Saving as YOLO OBB .txt')
                yolo_path = os.path.splitext(annotationFilePath)[0] + ".txt"
                print(f"Saving to: {yolo_path}")
                # print(f"Total shapes: {len(self.canvas.shapes)}")
                ...
                with open(yolo_path, "w") as f:
                    for shape in self.canvas.shapes:
                        if not shape.isRotated:
                            continue  # skip if not rotated box

                        # Assumes 4 points of rotated box
                        ...
                        label_id = 0  # or map label name to class ID here if needed
                        f.write(f"{label_id} {cx:.6f} {cy:.6f} {w:.6f} {h:.6f} {angle:.6f}\n")
                self.setClean()
### ### ###----------------------------------------------------------------------------------------                    
            elif selected_format == "YOLO-Corner":
                # print('Saving as YOLO OBB .txt')
                yolo_path = os.path.splitext(annotationFilePath)[0] + ".txt"
                print(f"Saving to: {yolo_path}")
                # print(f"Total shapes: {len(self.canvas.shapes)}")
                ...
                with open(yolo_path, "w") as f:
                    for shape in self.canvas.shapes:
                        ...
                        box = cv2.boxPoints(rect)  # returns 4 corners in float32
                        box = box.astype(np.float32)

                        # Normalize each point to image width and height
                        norm_box = []
                        for (x, y) in box:
                            norm_x = x / img_width
                            norm_y = y / img_height
                        ...
                        label_id = 0  # or map label name to class ID here if needed
                        # Save in format: class_id x1 y1 x2 y2 x3 y3 x4 y4
                        f.write(f"{label_id} " + " ".join([f"{v:.6f}" for v in norm_box]) + "\n")
...
# ### #### -------------------------- DINU EDIT -----------------------------------
    def saveFileDialog(self):
        ...
        if selected_format == "YOLO":
            filters = 'YOLO label file (*.txt)'
            default_suffix = 'txt'
        elif selected_format == "YOLO-Corner":
            filters = 'YOLO label file (*.txt)'
            default_suffix = 'txt'
        else:
            filters = 'Pascal VOC file (*.xml)'
            default_suffix = 'xml'
